<template>
    <div class="xy-tab"
         :class="{'xy-tab-no-animate': !animate}">
        <slot></slot>
        <div v-if="animate"
             class="xy-tab-ink-bar"
             :class="barClass"
             :style="barStyle">
            <span class="xy-tab-bar-inner"
                  :style="innerBarStyle"
...
<script>
import { parentMixin } from '../assets/mixins/multiItems';
...
export default {
    ...
    computed: {
        ...
        barClass() {
            return {
                'xy-tab-ink-bar-transition-forward': this.direction === 'forward',
                'xy-tab-ink-bar-transition-backward': this.direction === 'backward'
            }
        }
    },
    watch: {
...
.xy-tab.xy-tab-no-animate .xy-tab-item.xy-tab-selected {
    background: 0 0;
}
