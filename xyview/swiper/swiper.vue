<template>
  <div class="xy-slider">
    <div class="xy-swiper" :style="{height: xheight}">
      <slot></slot>
      <div class="xy-swiper-item" v-for="(item, index) in list" :key="index" @click="clickListItem(item)" :data-index="index">
        <a href="javascript:">
          <div class="xy-img" :style="{backgroundImage: buildBackgroundUrl(item.img)}"></div>
          <p class="xy-swiper-desc" v-if="showDescMask">{{item.title}}</p>
        </a>
      </div>
      <div v-if="listTwoLoopItem.length > 0">
        <div class="xy-swiper-item xy-swiper-item-clone" v-for="(item, index) in listTwoLoopItem" :key="index" @click="clickListItem(item)" :data-index="index">
          <a href="javascript:">
            <div class="xy-img" :style="{backgroundImage: buildBackgroundUrl(item.img)}"></div>
            <p class="xy-swiper-desc" v-if="showDescMask">{{item.title}}</p>
          </a>
    ...
    <div :class="[dotsClass, 'xy-indicator', 'xy-indicator-' + dotsPosition]" v-show="showDots">
      <a href="javascript:" v-for="key in length" :key="key">
        <i class="xy-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
...
import { go } from '../assets/libs/router'
...
@import '../assets/styles/variable'; // @pre: xy;
.xy-slider {
  overflow: hidden;
  position: relative;
